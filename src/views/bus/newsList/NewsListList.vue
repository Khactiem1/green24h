<template>
  <div class="container-table">
    <span @shortkey="shortkeyAction" v-shortkey="defaultShortKeyList"></span>
    <div class="container-table_header border-line-bottom">
      <div class="name-table text-uppercase">
        <h1>{{ $t("i18nNewsList.Title") }}</h1>
      </div>
    ...
    <div class="table-function sticky">
      ...
      <div style="min-width: 625px" class="table-function_search">
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const layoutTag = ref("NewsListList"); // Cấu hình layout
    const formDetailName = ref("NewsListDetail"); // Cấu hình tên form detail
    const subSystemCode = ref("news_list"); // Cấu hình phân quyền
    const api = ref(newsListAPI); // Cấu hình api
    const isFormMultipleLanguage = ref(true) // là form có đa ngôn ngữ
    const listColumnsMultipleLanguage = ref(['news_list_name']); // cấu danh sách các cột có đa ngôn ngữ để lấy tự động
