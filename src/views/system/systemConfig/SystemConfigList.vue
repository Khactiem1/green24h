  ...
  <div class="container-table">
    <div class="container-table_header border-line-bottom">
      <div class="name-table text-uppercase">
        <h1>{{ $t("i18nSystemConfig.Title") }}</h1>
      </div>
    ...
    <ms-grid-viewer
      ...
      :idField="'system_config_id'"
      :gridInfo="gridInfo"
      :loadData="loadData"
      @handleDoubleClickRow="edit"
    >
      <template #picture_value="{ record }">
        <div class="image-table">
          <img
            v-if="record.picture_value"
            v-bind:src="fileAPI.getFileViewUrl(record.picture_value)"
          />
        </div>
      </template>
      <template #boolean_value="{ record }">
        <div class="image-table">
          <ms-switch
            :disabled="true"
            v-model="record.boolean_value"
          ></ms-switch>
        </div>
      </template>
...
import useModuleSystemConfig from "@/stores/system/moduleSystemConfig";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import systemConfigAPI from "@/apis/system/systemConfigAPI";
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const layoutTag = ref("SystemConfigList"); // Cấu hình layout
    const formDetailName = ref("SystemConfigDetail"); // Cấu hình tên form detail
    const subSystemCode = ref("system_config"); // Cấu hình phân quyền
