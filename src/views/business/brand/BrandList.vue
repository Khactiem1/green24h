<template>
    <div class="container-table">
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'brand_id'" :gridInfo="gridInfo" :loadData="loadData"
            ...
            <template #featured="{ record }">
                <div class="image-table">
                    <ms-switch :disabled="true" v-model="record.featured"></ms-switch>
                </div>
            </template>
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const layoutTag = ref("BrandList"); // Cấu hình layout
        const formDetailName = ref("BrandDetail"); // Cấu hình tên form detail
