<template>
    <ms-dynamic-popup :width="800" :height="400" :title="`${editMode ? $ms.commonFn.getEnumResource(editMode, 'FormState') : ''
        } ${$t('i18nDistrict.Title')}`" @beforeOpen="beforeOpen">
                    ...
                    <div class="form-detail">
                        <div class="form-group">
                            <ms-input :rules="[{ name: 'required' }]" :maxLength="100"
                                :label="$t('i18nDistrict.Detail.district_name')" v-model="model.district_name">
                            </ms-input>
                        </div>
                        <div class="form-item_input">
                            <div class="form-group ms-small">
                                <ms-input :rules="[{ name: 'required' }]" :maxLength="10"
                                    :label="$t('i18nDistrict.Detail.district_code')" v-model="model.district_code">
                                </ms-input>
                            </div>
                            <div class="form-group ms-big">
                                <ms-combobox-v2 :label="$t('i18nDistrict.Detail.province_code')"
                                    ...
                                    v-model="model.province_code" v-model:display="model.province_name">
                                </ms-combobox-v2>
                            </div>
                        </div>
                    </div>
...
import useModuleDistrict from "@/stores/business/moduleDistrict";
...
export default defineComponent({
    ...
    setup() {
        ...
        const storeModule: any = useModuleDistrict(); // Cấu hình store
        const api = ref(districtAPI); // Cấu hình api
        const subSystemCode = ref("district"); // Cấu hình phân quyền
