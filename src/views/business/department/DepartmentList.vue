...
import useModuleDepartment from "@/stores/business/moduleDepartment";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import departmentAPI from "@/apis/business/departmentAPI";
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const layoutTag = ref("DepartmentList"); // Cấu hình layout
    const formDetailName = ref("DepartmentDetail"); // Cấu hình tên form detail
    ...
    const deleteRow = async (record: any) => {
      const me: any = proxy;
      if (record.is_parent) {
        showError(me.$t("i18nDepartment.ErrorDeleteParent"));
        return;
      }
