<template>
  ...
  import useModuleSupportList from "@/stores/business/moduleSupportList";
  import MenuItem from "@/components/dropdown/MenuItem.vue"; 
  import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
  import supportListAPI from "@/apis/business/supportListAPI";
  ...
  export default defineComponent({
    ...
    setup: () => {
      ...
      const layoutTag = ref("SupportListList"); // Cấu hình layout
      const formDetailName = ref("SupportListDetail"); // Cấu hình tên form detail
      ...
      const deleteRow = async (record: any) => {
        const me: any = proxy;
        if (record.is_parent) {
          showError(me.$t("i18nSupportList.ErrorDeleteParent"));
          return;
        }
