<template>
    <div class="container-table">
      ...
      >
        <template #is_used="{ record }">
          <div class="image-table">
            <ms-switch
              :disabled="true"
              v-model="record.is_used"
            ></ms-switch>
          </div>
        </template>
  ...
  export default defineComponent({
    ...
    setup: () => {
      ...
      const layoutTag = ref("VoucherList"); // Cấu hình layout
      const formDetailName = ref("VoucherDetail"); // Cấu hình tên form detail
