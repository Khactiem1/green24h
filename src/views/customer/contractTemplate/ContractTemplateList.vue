    ...
    <div class="container-table">
        <div class="container-table_header border-line-bottom">
            <div class="name-table text-uppercase">
                <h1>{{ $t("i18nContractTemplate.Title") }}</h1>
            </div>
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'contract_template_id'" :gridInfo="gridInfo" :loadData="loadData"
            @handleDoubleClickRow="edit">
            <template #file_url="{ record }">
                <ms-view-file v-model="record.file_url"></ms-view-file>
            </template>
...
import useModuleContractTemplate from "@/stores/customer/moduleContractTemplate";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import contractTemplateAPI from "@/apis/customer/contractTemplateAPI";
import MsViewFile from "@/components/input/MsViewFile.vue";
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const layoutTag = ref("ContractTemplateList"); // Cấu hình layout
        const formDetailName = ref("ContractTemplateDetail"); // Cấu hình tên form detail
