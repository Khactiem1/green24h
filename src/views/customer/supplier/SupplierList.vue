...
export default defineComponent({
    ...
    setup: () => {
         ...
         */
        const loadDataSupplierType = async () => {
            const me: any = proxy;
            const payload = {
                PageIndex: 0,
                PageSize: 0,
                Columns: "*",
                Filter: JSON.stringify(["dictionary_list_code", "=", "TrangThaiDuyet"]),
                Sort: "",
                CustomParam: {},
            };
            me.$ms.commonFn.mask();
        ...
        const customParamLoadData = (param) => {
            ...
            if (filterSupplierStatus.value != 'all') {
                const filterStatus = ["supplier_type", "=", filterSupplierStatus.value];
                me.$ms.commonFn.concatFilter(param, filterStatus);
            }
            param.Columns = `${param.Columns},logo`;
        };
        return {
            api,
            fileAPI,
