<template>
    <div class="container-table">
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'product_id'" :gridInfo="gridInfo" :loadData="loadData"
            @handleDoubleClickRow="edit">
            <template #picture="{ record }">
                <div class="image-table">
                    <img v-if="record.picture" v-bind:src="fileAPI.getFileViewUrl(record.picture)" />
                </div>
            </template>
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const storeModule: any = useModuleProduct(); // Cấu hình store
        const layoutTag = ref("ProductList"); // Cấu hình layout
        const formDetailName = ref("ProductDetail"); // Cấu hình tên form detail
        const subSystemCode = ref("product_list"); // Cấu hình phân quyền
        const api = ref(productAPI); // Cấu hình api
        /**
        ...
        const customParamLoadData = async (param: any) => {
            if (param.Sort) {
                param.Sort = `${param.Sort},product_name ASC,`;
            } else {
                param.Sort = "product_name ASC,";
            }
        };
        return {
            api,
