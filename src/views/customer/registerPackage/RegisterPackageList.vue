    ...
    <div class="container-table">
        <div class="container-table_header border-line-bottom">
            <div class="name-table text-uppercase">
                <h1>{{ $t("i18nRegisterPackage.Title") }}</h1>
            </div>
        </div>
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'register_package_id'" :gridInfo="gridInfo" :loadData="loadData"
...
import useModuleRegisterPackage from "@/stores/customer/moduleRegisterPackage";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import registerPackageAPI from "@/apis/customer/registerPackageAPI";
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const layoutTag = ref("RegisterPackageList"); // Cấu hình layout
        const formDetailName = ref("RegisterPackageDetail"); // Cấu hình tên form detail
