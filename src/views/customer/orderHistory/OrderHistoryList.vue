    ...
    <div class="container-table">
        <div class="container-table_header border-line-bottom">
            <div class="name-table text-uppercase">
                <h1>{{ $t("i18nOrderHistory.Title") }}</h1>
            </div>
        </div>
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'order_history_id'" :gridInfo="gridInfo" :loadData="loadData">
...
import useModuleOrderHistory from "@/stores/customer/moduleOrderHistory";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import orderHistoryAPI from "@/apis/customer/orderHistoryAPI";
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const storeModule: any = useModuleOrderHistory(); // Cấu hình store
        const layoutTag = ref("OrderHistoryList"); // Cấu hình layout
        const subSystemCode = ref("order_history"); // Cấu hình phân quyền
        const api = ref(orderHistoryAPI); // Cấu hình api
