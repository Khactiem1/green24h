<template>
    <div class="container-table">
        ...
        <ms-grid-viewer :ref="viewRef" :data="storeModule.items" :pageTotal="storeModule.total"
            :loading="storeModule.loading" :idField="'transaction_id'" :gridInfo="gridInfo" :loadData="loadData">
...
import useModuleTransaction from "@/stores/customer/moduleTransaction";
import MenuItem from "@/components/dropdown/MenuItem.vue";
import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
import transactionAPI from "@/apis/customer/transactionAPI";
...
export default defineComponent({
    ...
    setup: () => {
        ...
        const storeModule: any = useModuleTransaction(); // Cấu hình store
        const layoutTag = ref("TransactionList"); // Cấu hình layout
        const formDetailName = ref("TransactionDetail"); // Cấu hình tên form detail
        const subSystemCode = ref("transaction_history"); // Cấu hình phân quyền
        const api = ref(transactionAPI); // Cấu hình api
