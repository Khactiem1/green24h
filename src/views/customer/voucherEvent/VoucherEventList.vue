<template>
    <div class="container-table">
      <div class="container-table_header border-line-bottom">
        <div class="name-table text-uppercase">
          <h1>{{ $t("i18nVoucherEvent.Title") }}</h1>
        </div>
      ...
      <ms-grid-viewer
        ...
        :idField="'voucher_event_id'"
        :gridInfo="gridInfo"
        :loadData="loadData"
        @handleDoubleClickRow="edit"
      >
        <template #hide_voucher="{ record }">
          <div class="image-table">
            <ms-switch
              :disabled="true"
              v-model="record.hide_voucher"
            ></ms-switch>
          </div>
        </template>
  ...
  import useModuleVoucherEvent from "@/stores/customer/moduleVoucherEvent";
  import MenuItem from "@/components/dropdown/MenuItem.vue";
  import MenuWrapper from "@/components/dropdown/MenuWrapper.vue";
  import voucherEventAPI from "@/apis/customer/voucherEventAPI";
  ...
  export default defineComponent({
    ...
    setup: () => {
      ...
      const layoutTag = ref("VoucherEventList"); // Cấu hình layout
      const formDetailName = ref("VoucherEventDetail"); // Cấu hình tên form detail
